...
const { Bookings } = require('../config/db'); // We will store physical therapy bookings as standard bookings with type='physio'
const { verifyToken, verifyAdmin } = require('../middleware/auth');

// GET /api/physio - Admin: retrieve all physiotherapy bookings
router.get('/', verifyAdmin, (req, res) => {
  try {
    const list = Bookings.find({ type: 'physio' });
    res.json(list);
  } catch (err) {
...
router.get('/my', verifyToken, (req, res) => {
  try {
    const list = Bookings.find({ type: 'physio', userId: req.user.id });
    res.json(list);
  } catch (err) {
...
router.post('/', verifyToken, (req, res) => {
  ...
  try {
    ...
    if (therapist) {
      const isBooked = Bookings.findOne({ type: 'physio', date, slot: timeSlot, therapist });
      if (isBooked) {
...
router.put('/:id/status', verifyAdmin, (req, res) => {
  ...
  try {
    ...
    const updated = Bookings.findByIdAndUpdate(id, {
